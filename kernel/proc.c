...
found:
  ...
  p->priority = 0; // establecer mayor prioridad
  p->boost = 1;    //
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        // Verificar si la prioridad llega a los límites
        if (p->priority >= 9)
        {
          p->priority = 9; // Mantener la prioridad en 9 como máximo
          p->boost = -1;   // Cambiar el boost a -1 si llega a 9, así en el próximo ciclo la prioridad va disminuyendo
        }
        if (p->priority <= 0)
        {
          p->priority = 0; // Mantener la prioridad en 0 como mínimo
          p->boost = 1;    // Cambiar el boost a 1 si llega a 0, así en el próximo ciclo la prioridad va aumentando
        }
